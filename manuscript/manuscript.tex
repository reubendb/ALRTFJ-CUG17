...
New Jenkins projects (i.e. application tests) are required to be tested on our test and development system (TDS) and approved before deployment on Blue Waters. This is one of our key best practices. Once a test is deployed it is assigned to a specific Jenkins tab via the "Edit View" link associated with that tab. Tests that are still in development reside in the "All" tab view until they have been peer reviewed by at least one other test developer.
...
In working with Jenkins projects, we have found that it is useful to be aware of the following caveats: 
