...
In addition, we restrict connections to OTP using pwauth and session control which is also in the configuration file \texttt{/etc/httpd/conf.d/ssl.conf} inside the \texttt{<VirtualHost>} directive:
\begin{lstlisting}
...
OTP is configured by \texttt{/etc/pam.d/pwath}: 
\begin{lstlisting}
 #%PAM-1.0
